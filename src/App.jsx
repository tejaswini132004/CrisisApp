...
const App = () => {
  ...
  const [staffHotelInfo, setStaffHotelInfo] = useState(null);
  ...
  useEffect(() => {
    ...
    const savedCode = localStorage.getItem('gg_staff_hotel_code');
    const savedHotel = localStorage.getItem('gg_staff_hotel_info');
    if (savedCode && savedHotel) {
      setStaffHotelCode(savedCode);
      setStaffHotelInfo(JSON.parse(savedHotel));
    }
  }, []);
  ...
  const attemptLogin = () => {
    ...
    if (authStep === 'admin') {
      ...
      if (passkey === STAFF_PASS) {
        setPasskey('');
        setAuthStep('staff_hotel_code');
      } else setError('INVALID STAFF KEY');
    }
  };

  const verifyStaffCode = () => {
    setStaffCodeError('');
    const code = staffHotelCode.trim().toUpperCase();
    if (code.length < 4) { setStaffCodeError('Enter your hotel code'); return; }
    onValue(ref(db, 'hotel_codes/'), (snap) => {
      const data = snap.val();
      if (data && data[code]) {
        const info = { ...data[code], code };
        setStaffHotelInfo(info);
        localStorage.setItem('gg_staff_hotel_code', code);
        localStorage.setItem('gg_staff_hotel_info', JSON.stringify(info));
        if (!staffNameSaved) {
          setAuthStep('staff_name');
        } else {
          setUserRole('staff');
          setAuthStep(null);
        }
      } else {
        setStaffCodeError('Hotel code not found — ask your manager');
      }
    }, { onlyOnce: true });
  };
  ...
  // STAFF HOTEL CODE SCREEN
  if (authStep === 'staff_hotel_code') {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4 font-sans">
        <div className="w-full max-w-[420px]">
          <div className="bg-white border-2 border-blue-200 shadow-2xl p-8 rounded-3xl">
            <button onClick={() => { setAuthStep(null); setStaffCodeError(''); }} className="text-xs font-black text-gray-400 uppercase tracking-widest mb-6 hover:text-gray-900 transition-colors">← Back</button>
            <div className="text-center mb-8">
              <div className="text-5xl mb-3">👮</div>
              <h2 className="text-2xl font-black uppercase italic tracking-tighter text-gray-900">Staff Check-In</h2>
              <p className="text-xs text-blue-500 uppercase tracking-widest mt-2 font-black">Enter your hotel's staff code</p>
              <p className="text-[10px] text-gray-400 mt-1">Get this from your hotel manager</p>
            </div>
            <input autoFocus placeholder="HOTEL CODE" value={staffHotelCode}
              onChange={e => setStaffHotelCode(e.target.value.toUpperCase().slice(0, 8))}
              onKeyDown={e => e.key === 'Enter' && verifyStaffCode()}
              className="w-full bg-gray-50 border-2 border-gray-300 rounded-2xl p-5 text-center text-blue-600 font-mono font-black tracking-[0.5em] text-2xl focus:outline-none focus:border-blue-500 transition-all mb-3"
            />
            {staffCodeError && <p className="text-red-600 text-xs font-black uppercase tracking-widest text-center mb-3 animate-pulse">{staffCodeError}</p>}
            <button onClick={verifyStaffCode} className="w-full py-4 bg-blue-600 text-white rounded-2xl font-black uppercase text-sm tracking-widest hover:bg-blue-700 transition-all active:scale-95">
              Join Hotel →
            </button>
          </div>
        </div>
        ...
        <div className="w-full max-w-[420px]">
          {!authStep ? (
            <div className="bg-white border-2 border-gray-200 shadow-2xl p-8 sm:p-10 rounded-3xl">
              ...
              <div className="space-y-3">
                ...
                <button onClick={() => setAuthStep('staff')} className="w-full p-4 rounded-2xl bg-white border-2 border-gray-200 hover:border-blue-400 hover:bg-blue-50 transition-all text-left">
                  ...
                  {/* No name shown here — multiple staff use same device */}
                  <p className="text-[10px] text-gray-400 mt-1 font-medium italic">Security Personnel</p>
                </button>
            ...
            <div className="bg-white border-2 border-gray-200 shadow-2xl p-8 rounded-3xl">
              ...
              {/* FIXED: No hardcoded name on passkey screen — staff enters name AFTER hotel code */}
  ...
  return (
    <div className={`min-h-screen bg-white ${panicActive && userRole === 'staff' ? 'panic-bg' : ''}`}>
      ...
      {userRole === 'staff' && <StaffPage staffName={staffName} hotelInfo={staffHotelInfo} />}
    </div>
  );
};
